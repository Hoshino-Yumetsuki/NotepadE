...
/// Code page 850 — "Western European DOS (ibm850)".
const CP850: [u16; 128] = [
    0x00C7, 0x00FC, 0x00E9, 0x00E2, 0x00E4, 0x00E0, 0x00E5, 0x00E7,
    0x00EA, 0x00EB, 0x00E8, 0x00EF, 0x00EE, 0x00EC, 0x00C4, 0x00C5,
    0x00C9, 0x00E6, 0x00C6, 0x00F4, 0x00F6, 0x00F2, 0x00FB, 0x00F9,
...
/// Code page 852 — "Central European (ibm852)".
const CP852: [u16; 128] = [
...
/// Code page 865 — "Nordic DOS (IBM865)".
const CP865: [u16; 128] = [
    0x00C7, 0x00FC, 0x00E9, 0x00E2, 0x00E4, 0x00E0, 0x00E5, 0x00E7,
    0x00EA, 0x00EB, 0x00E8, 0x00EF, 0x00EE, 0x00EC, 0x00C4, 0x00C5,
    0x00C9, 0x00E6, 0x00C6, 0x00F4, 0x00F6, 0x00F2, 0x00FB, 0x00F9,
...
fn check_page(
    ...
    hand: &[u16; 128],
    dec: &[char; 128],
    enc: &oem_cp::OEMCPHashMap<char, u8>,
) {
    // decode: every high byte maps to the same scalar
    for i in 0..128usize {
        assert_eq!(
            dec[i] as u32, hand[i] as u32,
            "{name} decode byte 0x{:02X}",
            i + 0x80
        );
    }
    // encode: exhaustive BMP scan must agree with the hand reverse lookup
    for v in 0u32..=0xFFFF {
        let Some(c) = char::from_u32(v) else { continue };
        let hand_byte: u8 = if v < 0x80 {
            v as u8
        } else {
            hand.iter()
        ...
        let crate_byte = oem_cp::encode_char_lossy(c, enc);
        assert_eq!(
            crate_byte, hand_byte,
            "{name} encode U+{v:04X}: crate 0x{crate_byte:02X} vs hand 0x{hand_byte:02X}"
        );
    }
    // astral chars -> '?'
    assert_eq!(oem_cp::encode_char_lossy('\u{1F600}', enc), b'?', "{name} astral");
}

#[test]
fn oem_cp_tables_match_hand_tables() {
    use oem_cp::code_table::*;
    check_page("cp437", &CP437, &DECODING_TABLE_CP437, &ENCODING_TABLE_CP437);
    check_page("cp850", &CP850, &DECODING_TABLE_CP850, &ENCODING_TABLE_CP850);
    check_page("cp852", &CP852, &DECODING_TABLE_CP852, &ENCODING_TABLE_CP852);
    check_page("cp855", &CP855, &DECODING_TABLE_CP855, &ENCODING_TABLE_CP855);
